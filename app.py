...
# -------------------------------------------------------------
# MODE A: AUTHENTICATION ENGINE (ISOLATED EXECUTION LAYER)
# -------------------------------------------------------------
...
# -------------------------------------------------------------
# MODE B: THE ADVANCED CONTROL CENTER (ADMINISTRATION PORTAL)
# -------------------------------------------------------------
    ...
    with main_display_viewport:
        # HIGH-SPEED DATA OPTIMIZATION: Pull records ONCE globally for all tabs
        with st.spinner("Caching directory records..."):
        ...
        # ---- TAB 2: GLOBAL DIRECTORY CRUD CONTROLS ----
        with admin_tab[1]:
                ...
                if search_q == "" or (search_q in (f.get("head_of_family") or "").lower() or search_q in (
                    ...
                    with st.expander(f"🏡 {f.get('head_of_family')} | ഇല്ലം: {f.get('illam_name')}"):
                        ...
                        if not f.get("email_id"):
                            ...
                            if st.button("🔄 Reset Linked Email / Unlock Fallback Backdoor",
                                         key=f"rst_{f['family_id']}"):
        ...
        # ---- TAB 3: HIGH-SPEED AGE AUDIT CONTROLS ----
        with admin_tab[2]:
            ...
            if st.button("Calculate Voter Roll Registry (18+)"):
                # Runs smoothly because all_fams is pulled once at the top level
                with st.spinner("Calculating eligible voters..."):
        ...
        # ---- TAB 4: SYSTEM CONFIGURATION CREDENTIALS ----
        with admin_tab[3]:
...
# -------------------------------------------------------------
# MODE C: STANDARD USER / HOUSEHOLD PROFILE DASHBOARD
# -------------------------------------------------------------
else:
