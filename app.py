        ...
        if st.button("Log In with Email"):
            if input_email == "":
                ...
                if response.data and len(response.data) > 0:
                    ...
                    st.error(
                        "This email is not registered yet. If this is your first time onboarding, please click the link below.")
    ...
    # MODE B: BACKDOOR ONBOARDING (NAME + ILLAM)
        ...
        if st.button("Verify & Open Profile"):
            if input_head == "" or input_illam == "":
                ...
                response = supabase.table("families").select("id, head_of_family, email_id").ilike("head_of_family",
                                                                                                   f"%{input_head}%").ilike(
                    "illam_name", f"%{input_illam}%").execute()
                ...
                if response.data and len(response.data) > 0:
                    ...
                    if registered_email and str(registered_email).strip().lower() != 'null' and str(
                            registered_email).strip() != '':
                        st.error(
                            f"🛑 Access Restricted! This profile already has a registered email address ({registered_email}). You must log in using the Primary Email screen.")
                    else:
...
# ---- 2. LOGGED IN MEMBER PORTAL ----
    ...
    if not current_email or str(current_email).strip().lower() == 'null' or str(current_email).strip() == '':
        with st.status("📧 Phase 2 Security Setup Required", expanded=True):
            st.write(
                "We are setting up secure email authentication. Please link a primary email address for your household:")
                ...
                if "@" not in new_email or "." not in new_email:
                    ...
                    try:
                        ...
                        if "unique constraint" in str(e).lower():
                            st.error(
                                "This email address is already registered to another family profile. Please use a unique email.")
                        else:
